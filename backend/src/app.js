...
app.use((err, req, res, next) => {
  ...
  if (err.name === 'ZodError') {
    ...
    const message = errorDetails.map((e) => (e.field ? `${e.field}: ${e.message}` : e.message)).join(', ') || 'Validation failed';
  ...
  if (err.code === 'P2002') {
    const fields = Array.isArray(err.meta?.target) ? err.meta.target.join(', ') : (err.meta?.target || 'value');
    return res.status(409).json({
      success: false,
      message: `A user with this ${fields} already exists. Please provide a different ${fields}.`,
      error: {
        code: 'CONFLICT',
