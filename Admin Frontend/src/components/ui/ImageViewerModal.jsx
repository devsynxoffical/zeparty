...
export function ImageViewerModal({ isOpen, onClose, imageUrl, title = 'Profile Picture Preview', subtitle = '' }) {
  if (!isOpen || !imageUrl) return null;

  const handleDownload = () => {
    const a = document.createElement('a');
    a.href = imageUrl;
    a.download = title.replace(/\s+/g, '_').toLowerCase() + '.jpg';
    a.target = '_blank';
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={title}
      description={subtitle || 'Full resolution image view'}
      size="md"
    >
